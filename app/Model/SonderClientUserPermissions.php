 ...
 */
class SonderClientUserPermissions extends Base
{
    /**
     * SQL table name
     *
     * @var string
     */
    const TABLE = 'user_has_client_permissions';

    public function addClientUserPermission($clientID, $userID){
        if (!$this->existsClientUser($clientID, $userID)){
    ...
    public function removeClientUserPermission($clientID, $userID){
        return $this->db->table(self::TABLE)->eq('user_id', $userID)->eq('client_id', $clientID)->remove();
    }

    public function getClientUserPermissions(){
        return $this->db->table(self::TABLE)->findAll();
    }

    public function getClientsByUser($userId){
        return $this->db->table(self::TABLE)->eq('user_id', $userId)->findAll();
    }

    public function getUsersByClient($clientId){
        return $this->db->table(self::TABLE)->eq('client_id', $clientId)->findAll();
    }

    public function existsClientUser($clientId, $userId){
        return $this->db->table(self::TABLE)->eq('user_id', $userId)->eq('client_id', $clientId)->exists();
    }
//
//    /**
